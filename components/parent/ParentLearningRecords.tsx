...
type ReviewTargets = Record<StudentId, ReviewTargetMinutes | null>;
...
export function ParentLearningRecords() {
  ...
  useEffect(() => { const id = window.setTimeout(() => { setRecords(readLearningRecords()); setReviewTargets({ jiejie: getReviewTargetMinutes('jiejie', 'chinese'), meimei: getReviewTargetMinutes('meimei', 'chinese') }); }, 0); return () => window.clearTimeout(id); }, []);
