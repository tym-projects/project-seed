...
test('shares the 1/3/7 review state between variations in one review group', () => {
  const { deriveReviewState } = loadSpacedReviewModule();
  const result = JSON.parse(JSON.stringify(deriveReviewState({
    records: [
      record('variation-a', { createdAt: '2026-08-01T04:00:00.000Z' }),
      record('variation-b', { createdAt: '2026-08-02T04:00:00.000Z' }),
    ],
    student: 'jiejie', subject: 'chinese', questionId: 'variation-a',
    questionIds: ['variation-a', 'variation-b'],
    now: new Date('2026-08-02T05:00:00.000Z'), timeZone: 'Asia/Taipei',
  })));

  assert.deepEqual(result, {
    lastCompletedLocalDate: '2026-08-02', lastSessionHadWrong: false,
    stableSuccessStreak: 2, nextReviewLocalDate: '2026-08-05', isDue: false,
  });
});

test('resets a whole review group when either variation needed a retry', () => {
  const { deriveReviewState } = loadSpacedReviewModule();
  const result = JSON.parse(JSON.stringify(deriveReviewState({
    records: [
      record('variation-a', { createdAt: '2026-08-01T04:00:00.000Z' }),
      record('variation-b', { attempts: 2, firstAnswer: 1, createdAt: '2026-08-02T04:00:00.000Z' }),
    ],
    student: 'jiejie', subject: 'chinese', questionId: 'variation-a',
    questionIds: ['variation-a', 'variation-b'],
    now: new Date('2026-08-02T05:00:00.000Z'), timeZone: 'Asia/Taipei',
  ...
  assert.deepEqual(result, {
    lastCompletedLocalDate: '2026-08-02', lastSessionHadWrong: true,
    stableSuccessStreak: 0, nextReviewLocalDate: '2026-08-03', isDue: false,
  });
});

test('advances a review group only once when both variations are completed on the same day', () => {
  const { deriveReviewState } = loadSpacedReviewModule();
  const result = JSON.parse(JSON.stringify(deriveReviewState({
    records: [
      record('variation-a', { createdAt: '2026-08-01T04:00:00.000Z' }),
      record('variation-b', { createdAt: '2026-08-01T08:00:00.000Z' }),
    ],
    student: 'jiejie', subject: 'chinese', questionId: 'variation-a',
    questionIds: ['variation-a', 'variation-b'],
    now: new Date('2026-08-02T05:00:00.000Z'), timeZone: 'Asia/Taipei',
  ...
  assert.equal(result.stableSuccessStreak, 1);
  assert.equal(result.nextReviewLocalDate, '2026-08-02');
});

test('treats the approved third action variation as the same 1/3/7 learning group', () => {
  const { deriveReviewState } = loadSpacedReviewModule();
  const result = JSON.parse(JSON.stringify(deriveReviewState({
    records: [
      record('meimei-chinese-2', { student: 'meimei', createdAt: '2026-08-01T04:00:00.000Z' }),
      record('meimei-chinese-5', { student: 'meimei', createdAt: '2026-08-02T04:00:00.000Z' }),
      record('meimei-chinese-action-word-identification-3', { student: 'meimei', createdAt: '2026-08-02T08:00:00.000Z' }),
    ],
    student: 'meimei', subject: 'chinese', questionId: 'meimei-chinese-2',
    questionIds: ['meimei-chinese-2', 'meimei-chinese-5', 'meimei-chinese-action-word-identification-3'],
    now: new Date('2026-08-03T05:00:00.000Z'), timeZone: 'Asia/Taipei',
  })));

  assert.equal(result.stableSuccessStreak, 2);
  assert.equal(result.nextReviewLocalDate, '2026-08-05');
});

test('advances only once when a prior clean day is followed by primary and confirmation on one day', () => {
  const { deriveReviewState } = loadSpacedReviewModule();
  const result = JSON.parse(JSON.stringify(deriveReviewState({
    records: [
      record('variation-primary', { createdAt: '2026-08-01T04:00:00.000Z' }),
      record('variation-primary', { createdAt: '2026-08-02T04:00:00.000Z' }),
      record('variation-confirmation', { createdAt: '2026-08-02T08:00:00.000Z' }),
    ],
    student: 'jiejie', subject: 'chinese', questionId: 'variation-primary',
    questionIds: ['variation-primary', 'variation-confirmation'],
    now: new Date('2026-08-03T05:00:00.000Z'), timeZone: 'Asia/Taipei',
  })));
...
test('confirmation retry resets the whole group after a prior clean day', () => {
  const { deriveReviewState } = loadSpacedReviewModule();
  const result = JSON.parse(JSON.stringify(deriveReviewState({
    records: [
      record('variation-primary', { createdAt: '2026-08-01T04:00:00.000Z' }),
      record('variation-primary', { createdAt: '2026-08-02T04:00:00.000Z' }),
      record('variation-confirmation', { attempts: 2, firstAnswer: 1, createdAt: '2026-08-02T08:00:00.000Z' }),
    ],
    student: 'jiejie', subject: 'chinese', questionId: 'variation-primary',
    questionIds: ['variation-primary', 'variation-confirmation'],
    now: new Date('2026-08-03T05:00:00.000Z'), timeZone: 'Asia/Taipei',
  ...
  assert.equal(result.stableSuccessStreak, 0);
  assert.equal(result.lastSessionHadWrong, true);
  assert.equal(result.nextReviewLocalDate, '2026-08-03');
});
