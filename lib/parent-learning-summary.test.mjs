...
function loadModule() {
  ...
  const confirmationPath = new URL('./understanding-confirmation.ts', import.meta.url);
  const confirmation = { exports: {} };
  vm.runInNewContext(compile(readFileSync(confirmationPath, 'utf8')), {
    exports: confirmation.exports, module: confirmation, Intl,
  ...
  vm.runInNewContext(compile(readFileSync(modulePath, 'utf8')), {
    ...
    require: (name) => ({ '@/lib/spaced-review': spaced.exports, '@/lib/understanding-confirmation': confirmation.exports })[name],
  });
...
test('returns empty-safe periods for empty and invalid records', () => {
  ...
  assert.deepEqual(result.today, { completedRecordCount: 0, completedLearningGroupCount: 0, firstTryCorrectCount: 0, firstTryCorrectRate: null, retryRecordCount: 0 });
...
test('isolates Natural Science and Social Studies summaries for the same student', () => {
  const result = summary([
    record('natural', 'a', { subject: 'natural_science' }),
    record('social', 'b', { subject: 'social_studies' }),
  ], {
    subject: 'natural_science',
    questions: [{ id: 'a', question: '自然題' }],
  });

  assert.equal(result.today.completedRecordCount, 1);
  assert.equal(result.today.completedLearningGroupCount, 1);
});
...
test('dedupes groups and creates deterministic attention only after two retry records', () => {
  ...
  assert.deepEqual(result.attentionItems, [
    { kind: 'repeated-retry', unitId: 'group-a', label: '題目 A', retryRecordCount: 2, retryDateCount: 2 },
...
test('reports completed records separately from learning groups and requires retry dates to differ', () => {
  const result = summary([
    record('same-day-primary', 'a', { attempts: 1, createdAt: '2026-08-12T16:30:00.000Z' }),
    record('same-day-confirmation', 'b', { attempts: 2, createdAt: '2026-08-12T17:00:00.000Z' }),
    record('retry-next-day', 'a', { attempts: 2, createdAt: '2026-08-11T17:00:00.000Z' }),
  ]);
  assert.equal(result.today.completedRecordCount, 2);
  assert.equal(result.today.completedLearningGroupCount, 1);
  assert.equal(result.today.firstTryCorrectRate, 0.5);
  assert.equal(result.today.retryRecordCount, 1);
  assert.equal(result.last7Days.completedRecordCount, 3);
  assert.equal(result.last7Days.completedLearningGroupCount, 1);
  assert.deepEqual(result.attentionItems.filter((item) => item.kind === 'repeated-retry'), [
    { kind: 'repeated-retry', unitId: 'group-a', label: '題目 A', retryRecordCount: 2, retryDateCount: 2 },
  ]);
});

test('keeps records at the seven-local-day boundary and excludes the eighth day', () => {
  const result = summary([
    record('boundary', 'legacy', { createdAt: '2026-08-06T16:00:00.000Z' }),
    record('outside', 'legacy', { createdAt: '2026-08-06T15:59:59.000Z' }),
  ]);
  assert.equal(result.last7Days.completedRecordCount, 1);
});

test('separates pending confirmation from normal due items', () => {
  const result = summary([
    record('history-a', 'a', { createdAt: '2026-08-11T01:00:00.000Z' }),
    record('primary-today', 'a', { createdAt: '2026-08-13T01:00:00.000Z' }),
  ]);
  assert.deepEqual(result.pendingConfirmationItems, [{ unitId: 'group-a', label: '題目 A' }]);
  assert.equal(result.dueItems.some((item) => item.unitId === 'group-a'), false);
  assert.equal(result.attentionItems.some((item) => item.kind === 'due-review' && item.unitId === 'group-a'), false);
});

test('keeps parent due overview independent from the five-group review cap', () => {
  const manyQuestions = Array.from({ length: 6 }, (_, index) => ({ id: `q-${index}`, question: `題目 ${index}` }));
  const result = summary(manyQuestions.map((question) => record(`record-${question.id}`, question.id, { createdAt: '2026-08-11T01:00:00.000Z' })), { questions: manyQuestions });
  assert.equal(result.dueLearningUnitCount, 6);
  assert.equal(result.dueItems.length, 5);
});
