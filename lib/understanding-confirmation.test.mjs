...
test('does not trigger confirmation for a Mathematics singleton unit', () => {
  const { isConfirmationEligible } = loadModule();
  const singleton = { id: 'math-singleton', questions: [question('math-singleton')] };
  assert.equal(isConfirmationEligible({ group: singleton, records: [record('math-singleton', { subject: 'mathematics' })], student: 'jiejie', subject: 'mathematics', now, timeZone: 'Asia/Taipei' }), false);
});
...
test('does not trigger confirmation for Natural Science or Social Studies singleton units', () => {
  const { isConfirmationEligible } = loadModule();
  for (const subject of ['natural_science', 'social_studies']) {
    const id = `${subject}-singleton`;
    assert.equal(isConfirmationEligible({
      group: { id, questions: [question(id)] },
      records: [record(id, { subject })],
      student: 'jiejie',
      subject,
      now,
