...
test('selects Natural Science records only for the same student and subject', () => {
  const result = select([question('natural-one'), question('natural-two')], [
    record('natural-one', { subject: 'natural_science', createdAt: '2026-08-12T01:00:00.000Z' }),
    record('natural-two', { subject: 'social_studies', createdAt: '2026-08-11T04:00:00.000Z' }),
  ], { subject: 'natural_science' });

  assert.deepEqual(ids(result), ['natural-two']);
});
