...
test('provides the four natural and social banks with the approved questions', () => {
  ...
  assert.deepEqual(Object.fromEntries(Object.entries(all).map(([name, questions]) => [name, questions.length])), {
    jiejieNaturalScience: 8,
    jiejieSocialStudies: 6,
    meimeiNaturalScience: 4,
    meimeiSocialStudies: 6,
  });

  const questions = Object.values(all).flat();
  assert.equal(questions.length, 24);
  assert.equal(new Set(questions.map(({ id }) => id)).size, 24);
...
test('keeps each student and subject bank isolated to its approved question ids', () => {
  const expected = {
    jiejieNaturalScience: ['jiejie-natural-science-1', 'jiejie-natural-science-2', 'jiejie-natural-science-3', 'jiejie-natural-science-4', 'jiejie-natural-science-5', 'jiejie-natural-science-6', 'jiejie-natural-science-7', 'jiejie-natural-science-8'],
    jiejieSocialStudies: ['jiejie-social-studies-1', 'jiejie-social-studies-2', 'jiejie-social-studies-3', 'jiejie-social-studies-4', 'jiejie-social-studies-5', 'jiejie-social-studies-6'],
    meimeiNaturalScience: ['meimei-natural-science-1', 'meimei-natural-science-2', 'meimei-natural-science-3', 'meimei-natural-science-4'],
    meimeiSocialStudies: ['meimei-social-studies-1', 'meimei-social-studies-2', 'meimei-social-studies-3', 'meimei-social-studies-4', 'meimei-social-studies-5', 'meimei-social-studies-6'],
  };
...
test('validates unique answer indexes, option answers, and balanced answer positions', () => {
  ...
  assert.deepEqual(counts, [6, 7, 6, 5]);
});

test('keeps two question types per approved learning unit topic', () => {
  const questions = Object.values(banks).flatMap((path) => loadQuestions(path)).filter(({ id }) => {
    return !/[5678]$/.test(id);
  });
