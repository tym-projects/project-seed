...
test('provides the four Sprint 23 banks with exactly sixteen approved questions', () => {
  ...
  assert.deepEqual(Object.fromEntries(Object.entries(all).map(([name, questions]) => [name, questions.length])), {
    jiejieNaturalScience: 4,
    jiejieSocialStudies: 4,
    meimeiNaturalScience: 4,
    meimeiSocialStudies: 4,
  });

  const questions = Object.values(all).flat();
  assert.equal(questions.length, 16);
  assert.equal(new Set(questions.map(({ id }) => id)).size, 16);
  assert.ok(questions.every(({ options }) => options.length === 4));
  assert.ok(questions.every(({ reviewGroupId }) => reviewGroupId === undefined));
});

test('keeps each approved student and subject bank isolated to its four question ids', () => {
  const expected = {
    jiejieNaturalScience: ['jiejie-natural-science-1', 'jiejie-natural-science-2', 'jiejie-natural-science-3', 'jiejie-natural-science-4'],
    jiejieSocialStudies: ['jiejie-social-studies-1', 'jiejie-social-studies-2', 'jiejie-social-studies-3', 'jiejie-social-studies-4'],
    meimeiNaturalScience: ['meimei-natural-science-1', 'meimei-natural-science-2', 'meimei-natural-science-3', 'meimei-natural-science-4'],
    meimeiSocialStudies: ['meimei-social-studies-1', 'meimei-social-studies-2', 'meimei-social-studies-3', 'meimei-social-studies-4'],
  };
