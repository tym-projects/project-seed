...
test('Sprint 22 Mathematics banks contain exactly six approved units per student', () => {
  ...
  const originalJiejieIds = new Set(expectedJiejie.map((question) => question.id));
  const originalMeimeiIds = new Set(expectedMeimei.map((question) => question.id));
  const originalJiejie = jiejie.filter((question) => originalJiejieIds.has(question.id));
  const originalMeimei = meimei.filter((question) => originalMeimeiIds.has(question.id));

  assert.deepEqual(plain(originalJiejie), expectedJiejie);
  assert.deepEqual(plain(originalMeimei), expectedMeimei);
  assert.equal(new Set([...originalJiejie, ...originalMeimei].map((question) => question.id)).size, 12);
  assert.equal(originalJiejie.every((question) => !Object.hasOwn(question, 'reviewGroupId')), true);
  assert.equal(originalMeimei.every((question) => !Object.hasOwn(question, 'reviewGroupId')), true);
});
