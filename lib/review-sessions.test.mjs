...
test('keeps the original startedAt when a pending confirmation is reopened', () => {
  globalThis.window = { localStorage: createLocalStorage() };
  const { getOrCreateReviewSession } = loadReviewSessionsModule();
  const first = getOrCreateReviewSession(todayOptions());
  const reopened = getOrCreateReviewSession(todayOptions({ now: new Date('2026-08-12T18:30:00.000Z') }));
  assert.equal(reopened.startedAt, first.startedAt);
  assert.equal(reopened.localReviewDate, first.localReviewDate);
});
