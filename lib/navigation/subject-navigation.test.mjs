...
test('defines three explicit actions for each of the four subjects', () => {
  const { SUBJECT_NAVIGATION } = loadNavigationModule();

  assert.deepEqual(Array.from(SUBJECT_NAVIGATION, ({ subject, label, practice, review, reinforce }) => ({ subject, label, practice, review, reinforce })), [
    { subject: 'chinese', label: '國語', practice: '/chinese', review: '/review', reinforce: '/reinforce' },
    { subject: 'mathematics', label: '數學', practice: '/mathematics', review: '/mathematics/review', reinforce: '/mathematics/reinforce' },
    { subject: 'natural_science', label: '自然', practice: '/natural-science', review: '/natural-science/review', reinforce: '/natural-science/reinforce' },
    { subject: 'social_studies', label: '社會', practice: '/social-studies', review: '/social-studies/review', reinforce: '/social-studies/reinforce' },
  ]);
});
...
test('builds twelve student-scoped targets without defaulting non-Chinese actions to Chinese', () => {
  ...
  for (const [student, items] of [['jiejie', jiejie], ['meimei', meimei]]) {
    for (const item of items) {
      ...
      assert.deepEqual(Object.keys(item.actions).sort(), ['practice', 'reinforce', 'review']);
