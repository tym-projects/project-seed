...
test('confirmed monthly banks expose the expected eligible counts', () => {
  ...
  assert.equal(getFirstExamQuestionIds('jiejie', 'mathematics').length, 11);
  assert.equal(getFirstExamQuestionIds('jiejie', 'natural_science').length, 10);
  assert.equal(getFirstExamQuestionIds('jiejie', 'social_studies').length, 10);
  ...
  assert.equal(getFirstExamQuestionIds('meimei', 'social_studies').length, 8);
});
