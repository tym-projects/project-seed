...
test('keeps Chinese and Mathematics review targets isolated for the same student', () => {
  globalThis.window = { localStorage: createLocalStorage() };
  const settings = loadSettingsModule();
  settings.saveReviewTargetMinutes('jiejie', 'chinese', 10);
  settings.saveReviewTargetMinutes('jiejie', 'mathematics', 15);

  assert.equal(settings.getReviewTargetMinutes('jiejie', 'chinese'), 10);
  assert.equal(settings.getReviewTargetMinutes('jiejie', 'mathematics'), 15);
...
test('keeps Natural Science and Social Studies review targets isolated for the same student', () => {
  globalThis.window = { localStorage: createLocalStorage() };
  const settings = loadSettingsModule();
  settings.saveReviewTargetMinutes('jiejie', 'natural_science', 10);
  settings.saveReviewTargetMinutes('jiejie', 'social_studies', 15);

  assert.equal(settings.getReviewTargetMinutes('jiejie', 'natural_science'), 10);
  assert.equal(settings.getReviewTargetMinutes('jiejie', 'social_studies'), 15);
});
