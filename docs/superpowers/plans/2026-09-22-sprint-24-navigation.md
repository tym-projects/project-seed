...
- No production question bank, persisted schema, storage key, or core learning semantics changed. No commit or push has been made; the work is waiting for Final Human Review.
...
- [ ] 寫 failing test：載入設定並要求四科各有 `practice`、`review`、`reinforce`，且 route suffix 與核准 Design 完全一致。
- [ ] 執行 `node --test lib/navigation/subject-navigation.test.mjs`，預期因設定模組不存在而 RED。
- [ ] 建立最小 typed 設定，僅保存 subject、label、practice／review／reinforce suffix。
- [ ] 重跑 focused test，確認 GREEN。
- [ ] 完成條件：設定是唯一首頁入口來源，無未核准路由。
...
- [ ] 先擴充 failing test：對 `jiejie`、`meimei` 產生 12 個明確 targets，確認每一科三個 action 都指向正確 student prefix。
- [ ] 執行 focused test，確認現有固定國語 review／reinforce 導覽不符合四科 targets。
- [ ] 以共用設定渲染四個科目分組，每組顯示科目名稱與三個 link；保留既有首頁主題與回首頁行為。
- [ ] 執行 focused Node test 與 TypeScript check。
- [ ] 完成條件：兩個首頁各有 12 個明確入口，國語首次練習與既有 routes 不失效。
...
- [ ] 先新增 Browser tests：桌面檢查兩個首頁各 12 個 href、四科首次練習 route、國語既有 route 與無 console error。
- [ ] 執行 focused Browser test，在首頁固定國語 review／reinforce 版本上確認 RED。
- [ ] 加入 mobile viewport cases，確認四科入口可見、可點擊、無水平 overflow、可垂直捲動；僅作基本版面驗證。
- [ ] 修正後執行 `npm run test:browser -- tests/browser/sprint-24-navigation.spec.ts`，確認 GREEN。
- [ ] 完成條件：24 個入口在 disposable desktop/mobile contexts 可定位且 route targets 正確。
...
- [ ] 執行 `npm test`、`npm run lint`、`npx tsc --noEmit --incremental false`、`npm run build`、`npm run test:browser`、`git diff --check`。
- [ ] 記錄 Node／Browser／mobile viewport 實際結果、schema／storage 未變更與真實手機／孩子試用限制。
- [ ] 將 Sprint 24 標為 `Implementation complete / Awaiting Final Human Review`，不得標記 Completed。
- [ ] 完成條件：Git working tree 僅有預期 Sprint 24 變更，停在 Final Human Review Gate。
